...
fn get_latest_version() -> Result<(Version, String, String), String> {
    ...
    let release =
        ureq::get("https://api.github.com/repos/veeenu/sekiro-practice-tool/releases/latest")
            .call()
            .map_err(|e| format!("{}", e))?
...
fn perform_injection() -> Result<(), String> {
    ...
    dll_path.pop();
    dll_path.push("jdsd_sekiro_practice_tool.dll");

    if !dll_path.exists() {
        dll_path.pop();
        dll_path.push("libjdsd_sekiro_practice_tool");
